...
class RewardManager:
    ...
    def calculate_flag_capture_reward(self):
        ...
        """
        return self.observation["last_checkpoint_index"]-self.prev_observation["last_checkpoint_index"] if self.prev_observation else 0
    ...
    def calculate_distance_reward(self):
        ...
        """
        if not self.prev_observation:
            return 0
        obstacle=False
        target=self.observation["target_position"]
        cur_distance=np.linalg.norm(self.observation["target_position"])
        prev_distance=np.linalg.norm(self.prev_observation["target_position"])
        dd=cur_distance-prev_distance
        for obj_ in self.observation["terrain_grid"]:
            for obj in obj_:
                x,y=obj['relative_position']
                if np.sign(x)==np.sign(target[0]) and np.sign(y)==np.sign(target[1]) :
                    if obj['terrain_type']:
                        obstacle=True
        #print(dd)
        return min(1,-dd*5) if dd<0 else -0.5 if obstacle else -1
    ...
    def calculate_survival_reward(self):
        ...
        """
        return self.observation["agent_health_normalized"]
    ...
    def calculate_reward(self):
        ...
        return (self.calculate_flag_capture_reward()*5+
                self.calculate_distance_reward()+
                self.calculate_survival_reward()*10)
...
class MLPlay:
    ...
    def _predict_action(self, obs):
        obs_tensor = torch.as_tensor(obs).unsqueeze(0).cuda()
        with torch.no_grad():
