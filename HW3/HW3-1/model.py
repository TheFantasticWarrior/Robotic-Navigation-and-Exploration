...
class Preprocess(nn.Module):
    def __init__(self):
        ...
        self.point=nn.Linear(2,4)
        self.combine=nn.Linear(8*2+4*4,16)
        self.act=nn.ReLU()
    def forward(self,x):
        x1=self.act(self.pose(x[:,:6].reshape(-1,2,3)).reshape(-1,16))
        x2=self.act(self.point(x[:,6:].reshape(-1,4,2)).reshape(-1,16))
        x=torch.cat((x1,x2),-1)
...
class PolicyNet(nn.Module):
    ...
    def __init__(self, s_dim, a_dim, std=0.5):
        ...
        self.dist = ...
        """
        self.main=nn.Sequential(Preprocess(),
                                 nn.ReLU(),
                                 )

        self.dist=DiagGaussian(16,1)
