...
# Navigation
##############################
from navigation_utils import pos_int, render_path, render_dynamic_camera_and_minimap, render_velocity_plot, evaluate_and_draw_metrics,evaluate,draw_metrics
...
def navigation(args, simulator, controller, planner, start_pose=(100,200,0)):
    ...
    while(True):
        ...
        target = None
        if path is not None and collision_count == 0 and not has_finished:
            if args.simulator == "basic":
                ...
                next_lw = (next_v*180/np.pi-next_w*simulator.l)/simulator.model.r
                next_rw = (next_v*180/np.pi+next_w*simulator.l)/simulator.model.r
        ...
        #NewFeature
        nav_current_idx, has_finished = evaluate(simulator, path, nav_current_idx, cte_history, has_finished, sim_ticks)
        frameskip=1000
        if (sim_ticks+1)%frameskip==0 or has_finished:
            ...
            draw_metrics(
                simulator, path, cte_history, has_finished, sim_ticks, camera_view
            )
...
def parse_arguments():
    ...
    parser.add_argument("-lcs", "--lqr_control_state", type=str, default="steering_angle", choices=['steering_angle', 'steering_angular_velocity','a'], help="control state of LQR control of bicycle model")
    parser.add_argument("-is", "--init_shift", type=float, default=0.0, help="init location shift")
    parser.add_argument("-1", "--arg1", type=float, default=0.0, help="arg1")
    parser.add_argument("-2", "--arg2", type=float, default=0.0, help="arg2")
    ...
    try:
        if args.simulator == "basic":
            ...
            if args.controller == "pid":
                ...
                controller = Controller(model=simulator.model,kp=args.arg2,alpha=args.arg1)
            elif args.controller == "lqr":
